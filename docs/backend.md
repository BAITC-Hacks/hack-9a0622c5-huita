...
В режиме `openai` **OpenAI Responses API / GPT-6 Luna** предварительно выбирает
гипотезы по агрегатам данных. `GET /api/agent` показывает `engine=openai_python`,
модель, готовность и журнал расходов приложения. Политика сохраняется приватно и
передаётся worker без API-ключа. В режиме `local` остаётся автономный Python-алгоритм.
Ошибки OpenAI завершают запуск с понятным сообщением; скрытой подмены движка нет.
Конфигурация, кеш и ограничения: [agent.md](agent.md).
...
Лимиты аккаунтов OpenAI и Brev по $50 не считываются приложением. Для OpenAI ведётся
отдельный локальный журнал с лимитом $5 по умолчанию. Brev не используется.
Ключи провайдеров не передаются браузеру и процессу расчёта.
...
В `.env` выберите `BEESMART_AGENT_PROVIDER=openai` и заполните `OPENAI_API_KEY`.
Модель: `BEESMART_OPENAI_MODEL=gpt-6-luna`; лимит приложения: `BEESMART_LLM_BUDGET_USD=5`.
При пустом ключе `/api/agent` сообщает `ready=false`, новый запуск возвращает 503.
Для тестирования без сети задайте `BEESMART_AGENT_PROVIDER=local`. Новая настройка
вступает в силу после перезапуска. Автотесты используют подставной HTTP-ответ;
реальный платный вызов требует вашего ключа и доступа к модели.
