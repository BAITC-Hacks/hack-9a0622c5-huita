...
Это гибридный агент: OpenAI влияет на порядок исследования, а Python принимает последующие решения по наблюдениям. Итеративный LLM tool calling в этой реализации не используется. Результаты пилотов не отправляются модели для очередного решения. События `agent_planning` и `agent_policy_ready` отражают реальную подготовку политики или явно отмеченное попадание в кэш. Подставной HTTP-транспорт используется только в тестах.
